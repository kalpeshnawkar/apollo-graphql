...
exports.verifyOauth = async (parent, args, context) => {
    try {
        ...
        var userUpdate = await userModel.updateOne({ "gitID": payload.gitID }, { $set: { "gitVerify": true } })  // finding the user for the email provided and updating the verification field in the database  
        if (userUpdate) {
